...
class AutoClustering:
    ...
    def fit(self, X: np.ndarray, k_range: range = range(6, 13),
            min_cluster_size: int = 100, n_jobs: int = 1,
        ...
        Args:
            ...
            n_jobs: Nombre de cores CPU (1 = séquentiel, -1 = tous)
            use_feature_selection: Si True, sélectionne les meilleures features
            feature_names: Noms des features (requis si use_feature_selection=True)
        """
        # Standardisation
        ...
        if use_feature_selection and feature_names is not None:
            ...
            X_scaled, selected_names = self.select_optimal_features(
                X_scaled, feature_names, n_features=min(20, X.shape[1])
            )
            print(f"   {len(feature_names)} → {len(selected_names)} features sélectionnées")
        
        print(f"SEARCH: Clustering sur {X_scaled.shape[0]} échantillons, {X_scaled.shape[1]} features")
        ...
        for result in kmeans_results:
            if result:
                ...
                print(f"   k={result.n_clusters}: Silhouette={result.silhouette:.3f}, "
                      f"CH={result.calinski_harabasz:.0f}")
        
        # 2. Gaussian Mixture (GMM) (parallélisé)
        print("\nTEST: Test GMM...")
